...
public class ApiTest {
	...
	public void play_with_api(TestContext context) {
		...
		webClient.get("/api/token").putHeader("login", "foo") // <1>
				.putHeader("password", "bar").as(BodyCodec.string()) // <2>
				.send(tokenPromise);
		...
		Future<HttpResponse<JsonObject>> postPageFuture = tokenFuture.compose(tokenResponse -> {
			...
			webClient.post("/api/pages").putHeader("Authorization", jwtTokenHeaderValue) // <2>
					.as(BodyCodec.jsonObject()).sendJsonObject(page, promise);
		...
		Future<HttpResponse<JsonObject>> deletePageFuture = updatePageFuture.compose(response -> {
			...
			.as(BodyCodec.jsonObject()).send(promise);
			return promise.future();
		});
